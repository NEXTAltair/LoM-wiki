...
書かない (upstream 側 `docs/event/simple/` `docs/event/detailed_description/` の実際のページ構成が
根拠)。
...
-   **ファイル名(イベント名部分)は3ロケールすべてで完全に同一にする(必須)**：
    ...
    ja側だけファイル名を変えると言語切替リンクが404になる(パス不一致による事故の実績あり)。
    このため**ファイル名は中国語(原文)表記で統一する**
    (`docs/event/` の実ページ名 `唐門圍攻戰`・`眾人的決策`・`四師兄遠遊經商`、
    および ja 側の大半の既存ページ `1-04-1-大師兄找樂子.md` 等はこの規則の実例。
    表示タイトルの付け方は[よく使う書き方](/ja/develop/4-1-template)の命名節を参照)。
    日本語名をファイル名にしてよいのは、他ロケールに対応ページが存在しない
    ja 独自新設ページ (`2-02-4-お出かけ.md` 等) に限る。
-   「〜アーク」のような複数イベントをまとめた編集者独自の呼び名は upstream に対応物が無い
    (detailed_description は「1つの巨大な分岐ツリー」であって「複数イベントの連なりに editor が
    付けた呼び名」ではない)。既存ページでこの呼び名を使っている場合は解消対象。
...
-   **検証状態・出典の注記を書かない**：「(実測済み)」「実プレイで確認」
    「内部データの分析による判断」のような、情報の出所や検証状態を示す注記は
    公開ページに書きません (検証の記録は編集者のローカルな調査メモにとどめます)。
    ページには確認済みの結果だけを書き、未確認の点だけを `TODO:` として残します。
...
`## 基本流程` の直後に **カテゴリ**・**発生時期** (該当すれば複数ルートを箇条書きで) を示し、
...
1. **唐門会議：**
   + **📜議題：退席**：四師兄が永久に唐門を離れる
   + **📜議題：四師兄の遠行**：
     + **📖抉擇：もっともだ** → **📜議題：退席**：四師兄が永久に唐門を離れる
     + **📖抉擇：わざと難題を吹っかける**、**🎲天命：四師兄の反応......**(上限70、➕加値：唐惟元好感度)：
...
- 検証状態・出典の注記や内部 ID を書かないルール、TODO の扱いは
  [簡易表詳細テンプレート](#簡易表詳細-simple-テンプレート)の記法の約束と共通。
