...
ページ下部の「時系列で前/次」ナビゲーションは
[シナリオ簡易表](/ja/event/story-simple-table)の並び順から自動生成されるため、手で書く必要は
ありません ([公開後にやること](#公開後にやること)で簡易表に登録すれば繋がります)。
自動ナビが示すのは表の上での隣であって話の続きではないので、
**実際の前提イベント・後続イベントは従来どおり備考セルに書いてください**。
