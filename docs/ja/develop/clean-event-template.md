...
-   **🚧未実装マーカー**：ゲームデータに条件は入っているのに、対応する場面や会話が
    確認できない分岐に付けます。**発生する場所に、そのまま行内で書きます**
    (専用の節に隔離したり、ページ末尾へ移したりしません)。書いてよいのは
    「データ上こういう条件がある」「対応する場面は確認できていない」という事実だけで、
    「〜と推定される」の類は書きません。「見える差分が無い」と言い切る根拠
    (同じ場面の他の分岐は記事化されているのにこれだけ無い、対応する台詞が存在しない等) を
    取ってから付けること。判定値を書く場合はデータ上の値であると明記します
    (ゲーム内表示の好感度は内部値の 1/10 の端数切捨てで、尺度が違うため)。
